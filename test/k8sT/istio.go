...
var _ = Describe("K8sIstioTest", func() {
	...
	var (
		...
		prerelease     = "-beta.8"
		istioctlParams = "" +
			" --set values.pilot.image=docker.io/jrajahalme/istio_pilot:1.6.14-beta.8" +
			" --set values.global.proxy.image=docker.io/jrajahalme/istio_proxy:1.6.14-beta.8" +
			" --set values.global.proxy_init.image=docker.io/jrajahalme/istio_proxy:1.6.14-beta.8" +
	...
	Context("Istio Bookinfo Demo", func() {
		...
		It("Tests bookinfo inter-service connectivity", func() {
			...
			if !shouldConnect(reviewsPodV1.String(), "reviews", formatAPI(ratings, apiPort, health)) {
